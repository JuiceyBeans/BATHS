...
public class BlueAdmiral {
    ...
    public boolean commissionShip(String shipName) {
        ...
        for (Ship ship : reserveFleet) {
            if (ship.getName().equalsIgnoreCase(shipName) && ship.getStatus() == ShipState.RESERVE) {
    ...
    public boolean decommissionShip(String shipName) {
        ...
        for (Ship ship : squadron) {
            if (ship.getName().equalsIgnoreCase(shipName) && ship.getStatus() != ShipState.SUNK) {
                int refund = ship.getCost() / 2;  // Refund is half the cost
    ...
    public boolean restoreShip(String shipName) {
        ...
        for (Ship ship : squadron) {
            if (ship.getName().equalsIgnoreCase(shipName) && ship.getStatus() == ShipState.RESTING) {
                ship.setStatus(ShipState.ACTIVE);
                return true;
            }
        }
    ...
    public Ship findSuitableShip(Encounter encounter) {
        ...
        for (Ship ship : squadron) {
            if (ship.getStatus() == ShipState.ACTIVE && ship.canFight(encounter.getType())) {
                return ship;
            }
        }
